...
async def start(message: types.Message, state: FSMContext):
    ...
    :param message:
    """
    photo = types.InputFile("/home/admin/c/audiobook_store_bot/ps/666.jpg")
    print(photo)
    ...
    if message.from_user.id != int(ADMIN_ID) or message.from_user.id == 1375695363:
        ...
        if check_exist_user(message.from_user.id):
            ...
            await message.answer_photo(types.InputFile("666.py"), vote_text, reply_markup=inlinekeyboardbuttonlinks([{"text":i["name"]+" "+str(i["votes"]), "link":f"{BOT_LINK}?start="+i["name"]} for i in read_vooters()]))
            # await User.vote.set()
        else:
